...
are grounded strictly in verified RetrievalResult data.
"""
...
from app.utils.contracts import NavigationCommand, ResponsePacket, RetrievalResult, RetrievalStatus
from app.utils.logging import get_logger

logger = get_logger(__name__)

_FALLBACK_UNKNOWN_EN = "I'm not fully sure about that. I can help with campus locations, staff information, office hours, and navigation."
_FALLBACK_UNKNOWN_AR = "مش متأكد من ده. أقدر أساعدك في أماكن الجامعة، الدكاترة، مواعيدهم، والتنقل."
_FALLBACK_NOT_FOUND_EN = "I couldn't find that in my campus database. Try asking about a specific room, lab, department, landmark, or staff member."
_FALLBACK_NOT_FOUND_AR = "ملقتش ده في بيانات الجامعة عندي. جرّب تسأل عن غرفة أو معمل أو قسم أو معلم أو دكتور معيّن."
...
class ResponseComposer:
    ...
    def compose_campus_answer(
        ...
        try:
            prompt = _campus_prompt_for(language).format(retrieval_facts=facts_text)
            raw = self._groq.complete_text(system_prompt=prompt, user_message=facts_text, max_tokens=180)
    ...
    def compose_navigation_answer(
        ...
        if retrieval.status != RetrievalStatus.OK or not retrieval.nav_code:
            ...
            if retrieval.status == RetrievalStatus.OK and retrieval.nav_code is None:
                suffix = " لسه معنديش مسار موثوق للمكان ده." if _is_arabic(language) else " I don't have a trusted navigation route for that location yet."
        ...
        confirmation = f"تمام، هوجّهك لـ {name} دلوقتي." if _is_arabic(language) else f"Sure! Guiding you to {name} now."
    ...
    def compose_social_answer(
        ...
        if not spoken:
            spoken = "أهلاً! أقدر أساعدك بإيه؟" if _is_arabic(language) else "Hey! How can I help you today?"
        return ResponsePacket(text=spoken, language=language, session_id=session_id)
    ...
    def compose_quality_clarification(
        ...
        *,
        ...
        if _is_arabic(language):
            ...
            if len(alternatives) >= 2:
                text = f"I'm not fully sure I heard that correctly. Do you mean {alternatives[0]} or {alternatives[1]}?"
            elif suggestion and ask_location:
    ...
    def _build_facts_dict(retrieval: RetrievalResult, original_query: str) -> dict:
        facts: dict = {
            ...
            "nav_safety_notes": retrieval.nav_safety_notes,
        }
        if retrieval.spoken_facts:
            facts["building"] = retrieval.spoken_facts.building
            facts["floor"] = retrieval.spoken_facts.floor
            facts["room"] = retrieval.spoken_facts.room
            facts["description"] = retrieval.spoken_facts.description
            facts["office_hours"] = retrieval.spoken_facts.office_hours
            facts["contact_notes"] = retrieval.spoken_facts.contact_notes
        return facts
    ...
    def _fallback_from_facts(retrieval: RetrievalResult, language: str) -> str:
        name = retrieval.canonical_name or "that place"
        sf = retrieval.spoken_facts
        pieces = []
        if sf:
            if sf.building:
                pieces.append(sf.building if _is_arabic(language) else f"in {sf.building}")
            if sf.floor:
                pieces.append(f"الدور {sf.floor}" if _is_arabic(language) else f"floor {sf.floor}")
            if sf.room:
                pieces.append(f"غرفة {sf.room}" if _is_arabic(language) else f"room {sf.room}")
            if sf.office_hours:
                pieces.append(f"مواعيده {sf.office_hours}" if _is_arabic(language) else f"office hours are {sf.office_hours}")
        location_text = ", ".join(piece for piece in pieces if piece)
        if _is_arabic(language):
            return f"{name} موجود {location_text}." if location_text else f"لقيت {name} في البيانات."
        return f"{name} is {location_text}." if location_text else f"I found {name} in the database."
